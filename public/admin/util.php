...
function admin_layout(string $title, string $bodyHtml): void {
  ...
  echo "<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><link rel='icon' type='image/png' href='../images/Icon.png'>";
  ...
  echo "<style>
    ...
    a{color:inherit}
    .top{display:flex;gap:1rem;align-items:center;justify-content:space-between;padding:1rem 1.25rem;border-bottom:1px solid var(--border);background:rgba(7,12,21,0.9);position:static}
    ...
    .rt-wrap{display:grid;gap:.45rem}
    .rt-toolbar{
      position:sticky;
      top:5.1rem;
      z-index:20;
    ...
    .rt-toolbar .btn{padding:.4rem .65rem;font-weight:700}
    .rt-toolbar select{width:auto;min-width:120px}
    .rt-editor{
      min-height:140px;
      background:var(--card);
      border:1px solid var(--border);
    ...
    .rt-editor:focus{outline:1px solid var(--accent);border-color:var(--accent)}
    .rt-editor p{margin:.45rem 0}
    .rt-editor ul,.rt-editor ol{margin:.45rem 0 .45rem 1.2rem}
  ...
  echo "</div></div>";
  echo "<main class='wrap'>{$bodyHtml}</main>";
  echo "<script src='../theme-mode.js'></script>";
  echo "<script src='rich-text.js'></script>";
  echo "</body></html>";
}
